import {
  ...
  export class ProyeccionController {
    ...
    async generateIngredientReport(@Param('id') proyeccionId: string, @Res() res: Response) {
      try {
        // Generar el reporte usando el servicio
        const report = await this.proyeccionService.generateIngredientReportForProjection(proyeccionId);
